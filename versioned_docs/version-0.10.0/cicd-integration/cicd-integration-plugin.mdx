...
4. [__Getting a list of recommended tests to skip__](#getting-recommended-tests) - retrieves the list of tests that can be skipped as a JSON file (for use in custom CI/CD pipelines).

5. [__Sending Git metadata to Drill4J API__](#sending-git-metadata-to-drill4j-api) - to supply Drill4J with commit information (date, hash, author) from which a particular application version is built.
...
For the plugin to be able to send data to Drill4J backend, it needs to be configured with Drill4J API address and API key.
You can set these parameters in two ways:

1. __Option 1__ (recommended): using environment variables.
```shell
...
# Optional, HTTP request timeout in milliseconds (default: 60000)
DRILL_HTTP_TIMEOUT_MS="60000"
```

2. __Option 2__ (not recommended): using Gradle/Maven plugin configuration.
...
<plugin>
    ...
    <configuration>
        ...
        <!-- Optional, HTTP request timeout in milliseconds (default: 60000) -->
        <httpTimeoutMs>60000</httpTimeoutMs>
    </configuration>
</plugin>
```
...
The Gradle CI/CD integration plugin allows you to override any Drill4J configuration parameter for a specific Gradle task.
This can be used to:
...
<plugin>
    ...
    <configuration>
        ...
        <!-- Agent download configuration -->
        <agent>
            <version>${versions.javaAgent}</version>
        </agent>
        <!-- Enable desired features (see sections below) -->
        <coverage>
...
Drill4J scans application classes and methods to identify structural changes between application builds.
This can be done at either:
...
The Drill4J agent can scan application classes as they are loaded by the JVM during task execution (e.g., during application run time). 
...
drill {
    ...
    recommendedTests()
    // Setup baseline strategy for test recommendations
    ...
}
`}
 </Codeblock>
...
{`
drill {
    // Set to SEARCH_BY_TAG, SEARCH_BY_MERGE_BASE, SEARCH_BY_COMMIT or SEARCH_BY_BUILD_VERSION.
    // See "Baseline Strategy" section on the page below for more info
    baseline {
...
### Saving recommended tests to a file

This command produces a JSON file listing tests that can be safely skipped for the current build (based on impact analysis against the baseline).
Use it when you need the list of "tests to skip" as an artifact for a custom CI/CD pipeline that filters test execution outside of the Drill4J agent.

The output is written to `recommendedTests.json`:
- Gradle: `<project>/build/drill/recommendedTests.json`
- Maven: `<project>/target/drill/recommendedTests.json`

If this command is executed before running the tests, the Java Agent will detect the `recommendedTests.json` file at the expected path and use it as the source of tests to skip,
instead of requesting the list of recommended tests when the tests are run.

> When running the same configuration across several modules of a multi-module build,
> the plugin caches the HTTP response for identical parameters, so only one request is sent to the Drill4J backend.

#### Gradle

To retrieve the recommended tests, execute the following Gradle command:
```shell
./gradlew drillGetRecommendedTests
```

#### Maven

To retrieve the recommended tests, execute the following Maven command:
```shell
mvn drill:getRecommendedTests
```
...
drill {
    ...
    groupId = "my-group"
    appId = "my-application"
    // Optional, identifies the app version to compare. If not set, the current commit SHA is used.
    buildVersion = "1.0.0"
...
By default, the tag is resolved to a commit SHA and the baseline build is looked up by that commit.
Alternatively, the tag name itself can be interpreted as an application build version — use the `tagMatchBy` and `tagPrefix` options for that.
...
baseline {
    ...
    // Optional. "COMMIT_SHA" (default) — resolve tag to a commit and search by commit.
    // "BUILD_VERSION" — extract build version from the tag name and search by build version.
    tagMatchBy = "BUILD_VERSION"

    // Optional. Prefix stripped from the tag name to derive the build version (only used with tagMatchBy = "BUILD_VERSION").
    // E.g. for tag "v1.2.3" and tagPrefix = "v" the resulting build version is "1.2.3".
    tagPrefix = "v"
}
```
#### Maven
...
<baseline>
    ...
    <!-- Optional. COMMIT_SHA (default) — resolve tag to a commit and search by commit. -->
    <!-- BUILD_VERSION — extract build version from the tag name and search by build version. -->
    <tagMatchBy>BUILD_VERSION</tagMatchBy>
...
### Search by commit
This strategy compares the current build to the baseline identified by an explicit commit SHA.
Use it when the baseline commit is already known (e.g. it is provided by an upstream CI/CD stage).

#### Gradle
```kotlin
baseline {
    // Strategy to find a baseline build
    searchStrategy = "SEARCH_BY_COMMIT"

    // Commit SHA of the baseline build
...
<baseline>
    <!-- Strategy to find a baseline build -->
    <searchStrategy>SEARCH_BY_COMMIT</searchStrategy>

    <!-- Commit SHA of the baseline build -->
    <commitSha>a1b2c3d4e5f6</commitSha>
</baseline>
```

### Search by build version
This strategy compares the current build to the baseline identified by an application build version.
Use it when releases are tracked by a version string rather than by Git tags/commits.

#### Gradle
```kotlin
baseline {
    // Strategy to find a baseline build
    searchStrategy = "SEARCH_BY_BUILD_VERSION"

    // Build version of the baseline
    buildVersion = "1.0.0"
}
```
#### Maven
...
<baseline>
    <!-- Strategy to find a baseline build -->
    <searchStrategy>SEARCH_BY_BUILD_VERSION</searchStrategy>
