...
1. Download the following files and put them in the empty folder:
    ...
    - [docker-compose.yml](/files/0.10.0/deploy-drill4j-services/docker-compose.yml)
    - [.env](/files/0.10.0/deploy-drill4j-services/drill.env)
    - [db-init/01-create-metrics-db.sql](/files/0.10.0/deploy-drill4j-services/db-init/01-create-metrics-db.sql)

  Your folder structure should match the following:
    ```
    /db-init/01-create-metrics-db.sql
    .env
    docker-compose.yml    
    ```
...
3. [Metabase](http://localhost:8095) must display Metabase UI authentication form.
    ...
    - Default credentials: 
      - admin: `admin@admin.admin` password: `adminadmin1`
      - user: `user@user.user` password: `useruser1`
