...
# How To Use Metabase Dashboards 
...
1. Drill4J services deployed.
2. Application agent configured.
...
6. Lets go trough the specific metrics (these are available both in Isolated and Aggregate variants)
    ...
    Clicking on either __Code Coverage__ or __Methods__ charts will navigate you to __Build - Code Coverage__ dashboard
...
### __Recommended Tests__

<Image alt='Baseline set - Recommended Tests table appears' src="/img/0.9.0/metabase/build-2-baseline-set.PNG" />

12. Once __Baseline Build__ filter is set two things appear - __Baseline Build Info__ card (highlight №1) and __Recommended tests__ table (highlight №2). This table contains tests that Drill4J suggest you to execute in order to test changes between two application versions.

    How it works (by example):
    1. You have executed tests for version `0.1.0`.
    2. In these tests, test `"Make Order"` tested method `Cart.addItem`
    3. Method `Card.addItem` changed between two versions.
    4. Based on this information, Drill4J suggests you to execute test `"Make Order"` again.
    5. `"MakeOrder"` is _likely_ the best chance to test changed method. It is not guranteed though.
    6. Lets say you execute this test, but now its broken or insufficient. You'll notice that thanks to Drill4J's next metric.

### __Changed Or New Methods__
