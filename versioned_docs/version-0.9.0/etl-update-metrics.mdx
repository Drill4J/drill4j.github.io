...
**Note:** Applying changes to the environment variable requires restarting the Drill4J Backend instance for the new schedule to take effect.<br/>
**Best Practice:** Adjust the ETL schedule so that each run completes before the next one is triggered.
...
- Before querying impacted tests/methods
...
**Warning:** This operation is resource-intensive and can take a long time to complete.<br/>
...
**Note:** Changes to the environment variable require restarting the Drill4J Backend instance.<br/>
**Best Practice:**
...
You can track the real-time progress of ETL executions by querying the `etl_metadata` table in the `metrics` database schema.
This table provides detailed status and statistics for each ETL pipeline, extractor, and loader.

**Key columns:**
- `pipeline_name`: Name of the ETL pipeline
- `extractor_name`: Name of the data extractor component
- `loader_name`: Name of the data loader component
- `status`: Current status (STARTING, RUNNING, SUCCESS, FAILED)
- `duration`: Time taken for the last run \(in milliseconds\)
- `last_processed_at`: Timestamp of the last processed data item
- `last_run_at`: Timestamp when the last run started
- `rows_processed`: Number of rows processed in the last run
...
- Data processing lag growing
- Metrics not reflecting recent data
