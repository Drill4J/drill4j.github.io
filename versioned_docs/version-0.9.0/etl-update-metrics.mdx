...
```json
"data": {
  "status": "RUNNING", // Current ETL status
...
**ETL Statuses:**
- `STARTING`: ETL process is extracting data from `raw_data` schema.
- `RUNNING`: ETL is actively loading data.
- `SUCCESS`: ETL completed successfully.
- `FAILED`: ETL encountered an error and stopped.
