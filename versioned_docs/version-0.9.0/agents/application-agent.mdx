...
> TIP: if you are setting parameters with env variables, prefix parameter name with `DRILL_`. For example `apiUrl` will become `DRILL_API_URL`

Although __buildVersion__ and __commitSha__ are not strictly required, we recommend providing at least one (ideally both) because:
1. It enables Drill4J to group metrics by version or commit.
2. Without them, metrics are reported separately for each instance, preventing aggregation.
