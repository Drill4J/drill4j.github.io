...
Drill4J Java Agent is a unified runtime agent for JVM-based applications that provides deep observability into your testing process and codebase evolution.

Key capabilities:

- **Code Coverage Measurement** — collects precise method- and line-level code coverage data from running applications, giving teams real-time visibility into which parts of the codebase are exercised by tests.
...
- **Context Propagation** — automatically propagates test context across threads, asynchronous calls, HTTP requests, messaging systems (Kafka), and WebSocket connections, ensuring accurate coverage attribution even in distributed and reactive architectures.
- **Code Change Detection** — identifies structural changes between application builds at the method level by comparing `commitSha` and `buildVersion` metadata, powering intelligent test recommendations that highlight only the tests affected by recent changes.
...
Refer to [next section](#how-to-set-path-to-agent) for further steps.
...
To load Drill4J Java Agent, you need to instruct Java Virtual Machine (JVM) to use agent files.
Drill4J has two modes of loading:
...
To load Drill4J Java Agent in Java mode, use the following command line argument:
...
| buildVersion         | recommended | `v1.2.3`                              | Build version of your application. Typically set to version tag. [See more](#considerations-regarding-buildversion-and-commitsha).                                          |
| commitSha            | recommended | `8d87b0c2379a925...`                  | Full SHA hash of commit from which your application .jar is built from. [See more](#considerations-regarding-buildversion-and-commitsha).                                   |
...
| scanClassPath          | no       | `build/libs/myapp-0.0.1.jar`             | Path(s) to application's .class/.jar/.war/.ear files. If none are specified, the agent attempts to find application classes automatically. See [syntax and examples](#how-to-set-scanclasspath) below. |
...
| testSessionId        | no       | `test-session-1`                         | Explicit test session identifier. If set, the agent uses this value instead of generating a session ID automatically. |
...
| instrumentationWsEnabled         | no       | `true`  | Enables instrumentation for WebSocket connections.               |
...
Useful when the agent is unable to locate these automatically (e.g. when running [Wildfly](/docs/guides/wildfly) or [Tomcat](/docs/guides/tomcat)).
...
JaCoCo conflicts with the Drill4J Java Agent, so it needs to be removed.
This shouldn't be a concern, as Drill4J includes the same functionality along with additional features.
