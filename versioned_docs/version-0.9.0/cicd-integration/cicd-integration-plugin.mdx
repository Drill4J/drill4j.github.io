...
To do this, use the `drillScanAppArchive` task in Gradle or the `drill:scanAppArchive` goal in Maven.
...
To scan the information about the application's classes and methods and send it to Drill4J Backend, execute the following Gradle command:
```shell
./gradlew drillScanAppArchive
```
### Maven

To scan the information about the application's classes and methods and send it to Drill4J Backend, execute the following Maven command:
```shell
