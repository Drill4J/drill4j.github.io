...
1. Identifies changes in a branch between the __latest commit__ and the configured [__baseline commit__](#configure-baseline-strategy). Generally, the __baseline commit__ is a tagged commit that follows the `tagPattern` defined in the [configuration file](#generate-report-on-branch-push).
...
Next, you need to configure the command to generate reports on Pull Requests.
