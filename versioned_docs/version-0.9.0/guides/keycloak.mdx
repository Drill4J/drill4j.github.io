...
- A running instance of Drill4J Backend.
...
1. **Set environment variables**: You need to set the following environment variables for Drill4J Backend to connect to Keycloak:
   ...
   - Set `DRILL_OAUTH2_TOKEN_MAPPING_USERNAME` to `preferred_username`.
...
2. **Login with Auth Provider**: If you have not disabled Simple Authentication, click the "Sign in with Auth Provider" button in the login window, otherwise you will be automatically redirected to Keycloak.
