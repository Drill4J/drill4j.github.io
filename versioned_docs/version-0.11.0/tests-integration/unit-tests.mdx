...
Drill4J CI/CD Integration Plugin allows to enable the Drill4J agent to capture metrics from Unit tests execution.
...
To enable the Drill4J agent add the following lines to configuration:
...
drill {
    ...
    // Enable coverage collection and test tracing
    coverage()
    testTracing()
}
`}
