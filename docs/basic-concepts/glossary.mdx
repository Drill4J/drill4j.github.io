...
- **<i id="admin-backend">Admin Backend</i>** - the core Drill4J backend service which stores and processes information about all [_Agents_](#agent).
  <br />
  <br />
- **<i id="agent">Agent</i>** - a component (or set of components) responsible for **tracking code execution** during tests and reporting this data to [_Admin Backend_](#admin-backend). Each such _Agent_ has a corresponding [_Agent_](#agent-entity) entity.
  <br />
  <br />
- **<i id="agent-java">Java Agent</i>** - [an agent implementation for Java/JVM applications](https://github.com/Drill4J/java-agent) which integrates with the target [_Service_](#service) through [JVM TI](https://docs.oracle.com/javase/8/docs/technotes/guides/jvmti/).
  <br />
  <br />
- **<i id="autotest-agent">Autotest Agent</i>** - a component that **integrates with the testing framework**. It extracts [_Test context_](#test-context), then sends it to the [_Admin Backend_](#admin-backend) and propagates it through test actions (e.g. by adding custom headers to HTTP request). Learn more in [general architecture](/docs/basic-concepts/general-architecture) section.
  <br />
  <br />
