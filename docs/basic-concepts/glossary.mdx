...
- **<i id="tests-api">API tests</i>** - tests which call API endpoints directly.
  <br />
  <br />
- **<i id="tests-ui">UI tests / E2E tests</i>** [\*](#ui-tests-note) - tests which manipulate web browser, simulating user actions.
  <br />
  <br />
- **<i id="tests-manual">Manual tests</i>** - tests performed manually in the web browser.
  <br />
  <br />
- **<i id="tests-unit">Unit tests</i>** - tests focused and executed on isolated parts of a code. The important thing is that the whole application is not being built or launched, but only specific bits.
  <br />
  <br />
...
- **<i id="admin-backend">Admin Backend</i>** - the core Drill4J backend service which stores and processes information about all [_Agents_](#agent).
  <br />
  <br />
- **<i id="agent">Agent</i>** - a component (or set of components) responsible for **tracking code execution** during tests and reporting this data to [_Admin Backend_](#admin-backend). Each such _Agent_ has a corresponding [_Agent_](#agent-entity) entity within Drill4J Admin Panel.
  <br />
  <br />
- **<i id="agent-java">Java Agent</i>** - [native agent](https://github.com/Drill4J/java-agent) for JVM based on [JVM TI](https://docs.oracle.com/javase/8/docs/technotes/guides/jvmti/) integrated with the target [_Service_](#service).
  <br />
  <br />
- **<i id="autotest-agent">Autotest Agent</i>** - a component that **integrates with testing framework**. It extracts [_Test context_](#test-context), then sends it to the [_Admin Backend_](#admin-backend) and propagates it through test actions (e.g. by adding custom headers to HTTP request). Learn more in [general architecture](/docs/basic-concepts/general-architecture) section.
  <br />
  <br />
- **<i id="java-autotest-agent">Java Autotest Agent</i>** - [autotest agent implementation](https://github.com/Drill4J/autotest-agent) written for tests implemented in Java or other JVM language. It as available both as a [Maven or Gradle plugin](/docs/installation/setup-java-autotests).
  <br />
  <br />
...
- **<i>Agent Registration</i>** - a one-time configuration process. Whenever the [_Agent_](#agent), previously unknown to Drill4J, is added, user must navigate to [_Admin Panel_](#admin-ui) and go through registration procedure.
  <br />
  <br />
- **<i id="build">Build</i>** - is a concrete version of the [_Service_](#service), identified by _Build version_ string. _Build_ is always associated with the _Agent_.
  <br />
  <br />
- **<i id="packages-classes-methods">Packages, Classes and Methods</i>** - for Java and JVM languages (as well as .NET) all these terms are exactly what they are.
  **However** for JavaScript/TypeScript these terms do not always make much sense. But to keep UI consistent, we stuck with these terms, with the [following caveats](/docs/basic-concepts/basics-javascript#caveats).
  <br />
  <br />
