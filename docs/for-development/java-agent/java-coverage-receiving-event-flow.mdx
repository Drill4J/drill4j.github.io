...
title: Coverage Receiving Event Flow
---
...
### Flow
1. User executes HTTP request to start session.
2. Admin does an action **StartNewSession**.
3. Admin's plugin part starts session on its side and sends **StartAgentSession** to agent's plugin part.
...
6. If the started session has realtime enabled:
...
### Sequence Diagram
<Image src="../assets/coverage-receiving-sequence.png"/>
