...
| **supportedTechnologies**                                     | An array of supported technologies in application under test. Currently, the only available value is **HttpClient**.                                                                               |
...
<Warning>
    The Scanner application utilazes Coverlet for code coverage, which relies on Program Database Files (PDB). 
    These PDB files contain information about the original paths of the source code files.
    Therefore, ensure that source files remain at their original location after the project build 
    or rebuild your project after moving the files, so that the PDB files reflect the new paths of the source code files.
</Warning>
